...
#-------------------------------------WaveMix----------------------------------------#
def _compute_scale_factor(original_width, original_height, min_dimension_multiple_of_16):
  if not min_dimension_multiple_of_16:
    return 1.0

  # Compute the target dimensions that are multiples of 16
  target_width = int(np.ceil(original_width / 32) * 32)
  target_height = int(np.ceil(original_height / 32) * 32)

  # Calculate the scale factor based on the target dimensions
  if original_width > original_height:
    scale_factor = min_dimension_multiple_of_16 / target_height
  else:
    scale_factor = min_dimension_multiple_of_16 / target_width
  return scale_factor


#-------------------------WaveMix-------------------------------------------------#
def load_image(url, preprocessing, min_dimension_multiple_of_16=None, horizontal_flip=False):
  ...
  if min_dimension_multiple_of_16 is not None:
    scale_factor = _compute_scale_factor(
      original_width=original_width,
      original_height=original_height,
      min_dimension_multiple_of_16=min_dimension_multiple_of_16
    )
    width = int(image.width * scale_factor)
    height = int(image.height * scale_factor)

    # Adjust the dimensions to be multiples of 16
    width = int(np.ceil(width / 32) * 32)
