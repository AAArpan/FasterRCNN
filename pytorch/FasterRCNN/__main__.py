...
def train(model):
  ...
  if options.load_from:
    ...
    if options.backbone == "vgg16":  # "vgg16" is our hand-implemented backbone and the only one without built-in default weights
      ...
      initial_weights = "WAVEMIX_IMAGENET"
...
if __name__ == "__main__":
  ...
  valid_backbones = [ "vgg16", "vgg16-torch", "resnet50", "resnet101", "resnet152", "WaveMix"]
  ...
  elif options.backbone == "WaveMix":
    backbone = WaveMix.WaveMixBackbone(dropout_probability = options.dropout)
